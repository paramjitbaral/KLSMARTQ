...
const OfficeForm: React.FC<{
    office?: Office;
    onSave: (office: Omit<Office, 'id'> | Office) => Promise<void>;
    onCancel: () => void;
}> = ({ office, onSave, onCancel }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsSaving(true);
        try {
            await onSave(office ? {...office, ...formData} : formData);
        } catch (err: any) {
            ...
            <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md">
                 ...
                 <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end space-x-4 pt-4">
                        <button type="button" onClick={onCancel} disabled={isSaving} className="px-6 py-2 bg-neutral-200 text-neutral-800 font-semibold rounded-lg hover:bg-neutral-300 transition-colors disabled:opacity-50">Cancel</button>
                        <button type="submit" disabled={isSaving} className="px-6 py-2 bg-primary text-white font-semibold rounded-lg hover:bg-primary-light transition-colors disabled:bg-neutral-400 w-28 flex justify-center items-center">
                            {isSaving ? (
...
const OfficeManagementPage: React.FC = () => {
  ...
  const handleSave = async (officeData: Omit<Office, 'id'> | Office) => {
    if ('id' in officeData) {
      await updateOffice(officeData);
    } else {
