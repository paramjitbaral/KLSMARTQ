...
const Sidebar: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    const handleLogout = async () => {
        ...
        try {
            await logout();
            // The component will unmount on successful logout, so no need to reset state
        } catch (error) {
        ...
        <aside className="w-64 bg-primary-dark text-white flex flex-col p-4 h-full">
            ...
            <div>
                 <button 
                    ...
                    className="w-full flex items-center justify-center space-x-3 px-4 py-3 rounded-lg text-neutral-200 hover:bg-red-500 hover:text-white transition-colors duration-200 disabled:bg-red-800 disabled:cursor-not-allowed"
                >
